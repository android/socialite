...
internal class StyleTransferEffect(context: Context, styleAssetFileName: String) :
    ...
    init {
        ...
        val predictInterpreter =
            Interpreter(FileUtil.loadMappedFile(context, predictModel), options)
    ...
    override fun getScaledRegion(presentationTimeUs: Long): GlRect {
        return GlRect(0, 0, inputWidth / 2, inputHeight)
    }
    ...
    private fun getScaledTensorImage(
        bitmap: Bitmap,
        targetWidth: Int,
        targetHeight: Int,
    ): TensorImage {
    ...
    private fun copyBitmapToFbo(bitmap: Bitmap, textureInfo: GlTextureInfo, rect: GlRect) {
        ...
        GlUtil.blitFrameBuffer(
            fboId,
            GlRect(0, 0, bitmap.width, bitmap.height),
            textureInfo.fboId,
            rect,
        )
